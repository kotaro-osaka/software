...
public class Kundenstamm {
	private static final short ANZAHL_KUNDEN = 59;
	
	protected static final short MYSQL_IDX = 1;
	protected static final short SQLITE_IDX = 2;
    private static final String MYSQL_URL = "jdbc:mysql://localhost:3306/amnetprime_kunden";
    private static final String SQLITE_URL = "jdbc:sqlite:H:\\HeidiSQL_11.1_64_Portable\\amnetprime.db";
    ...
    public static void setDatabase(short database) {
    	switch ((int) database) {
    	case 1: 
    		activeURL = MYSQL_URL;
    		break;
    	case 2:
    		...
    		break;
    	default: throw new IllegalArgumentException("Unexpected value: " + database);
    	}
    }
    
    public static short getAnzahlKundenFromDb(String query) {
    	String countQuery = "SELECT COUNT(*) AS row_count FROM (" + query + ") AS dynamic_query;";
    	short rows = -1;
    	
        try (Connection conn = DriverManager.getConnection(activeURL, USER, PASSWORD);
    ...
    public static Kunde[] liesKundenFromDb(short database, String query) {
    	setDatabase(database);
    	
        short rows = getAnzahlKundenFromDb(query);

        Kunde[] kundenliste = new Kunde[rows];
        ...
        try (Connection conn = DriverManager.getConnection(activeURL, USER, PASSWORD);
            ...
            while (rs.next()) {
                ...
                kundenliste[kundenlisteIdx] = new Kunde(
                    kundennummer, vorname, nachname, "01.01.1901", "", plz, "", "01.01.1901", geschlecht, familienstand, 0
                );
